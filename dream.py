...
f = factors(size)
os.system("convert -size \"%sx%s\" -depth 8 rgba:outputs/images/%s.raw outputs/images/%s.png" % (f[len(f)/2], size/f[len(f)/2], name, name))
os.system("python color.py outputs/images/" + name + ".png " + transform)
size2 = os.path.getsize("outputs/images/%s.png" % name) / 3
f = factors(size2)
os.system("convert -size \"%sx%s\" -depth 8 outputs/images/%s.png rgba:outputs/images/%s.raw" % (f[len(f)/2], size2/f[len(f)/2], name, name) )
os.system("convert -size \"%sx%s\" -depth 8 outputs/images/%s.png rgba:outputs/images/%s.jpg" % (f[len(f)/2], size2/f[len(f)/2], name, name) )
